...
public class GuiOptimiser {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 5; i++) // RunTargetApp runTargetApp = new RunTargetApp(parentDir.concat(TARGET_APP),
                                // TARGET_APP_RUNNINGTIME);
    {
  ...
  public static void runApp(String path, int targetAppRunningtime) {
    try {
      // java -jar
      // C:\Users\Mahmoud-Uni\Documents\NetBeansProjects\calculator\dist\calculator.jar
      ...
      String bestOne = "";
      Double bestOneCharge = 999999.0;
      try {
        Thread.sleep(targetAppRunningtime);
        Capture capture = new Capture();
        String screenShoot = capture.takeScreenShoot();
        Double chargeConsumption = Util.calculateChargeConsumptionForImage(screenShoot);
        if (chargeConsumption < bestOneCharge) {
          bestOne = screenShoot;
          bestOneCharge = chargeConsumption;
        }
  ...
  public static void changeColorAll() {
    try {
      ...
      RGB.add(new ArrayList<Integer>(
          Arrays.asList(new Integer[] { randomInt.nextInt(256), randomInt.nextInt(256), randomInt.nextInt(256) })));
      RGB.add(new ArrayList<Integer>(
          Arrays.asList(new Integer[] { randomInt.nextInt(256), randomInt.nextInt(256), randomInt.nextInt(256) })));
      RGB.add(new ArrayList<Integer>(
          Arrays.asList(new Integer[] { randomInt.nextInt(256), randomInt.nextInt(256), randomInt.nextInt(256) })));
  ...
  public static void saveToCSV(String filePath, ArrayList<String> guiComponents, ArrayList<ArrayList<Integer>> RGB) {
    try {
      ...
      for (int i = 0; i < guiComponents.size(); i++) {
        line += guiComponents.get(i).concat(",")
            .concat(RGB.get(i).toString().replace("[", "").replace("]", "").replaceAll("\\s", "")) + "\n";
        // System.out.println(line);
      }
      br.write(line);
      br.flush();
